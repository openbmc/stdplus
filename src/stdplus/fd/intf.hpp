...
class Fd
{
  public:
    ...
    void readAll(stdplus::span<std::byte> data);
    void recvAll(stdplus::span<std::byte> data, int flags);
    void writeAll(stdplus::span<const std::byte> data);
    void sendAll(stdplus::span<const std::byte> data, int flags);
};
