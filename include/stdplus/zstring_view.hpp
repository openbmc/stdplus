...
struct compile_zstring_view
{
    ...
    {
        return unsafe_zstring_view(std::basic_string_view<CharT>(data, N));
    }
};
...
class basic_zstring_view
{
  ...
  public:
    ...
    inline constexpr basic_zstring_view(string_view_base sv) : sv(sv)
    {
        if (sv.find('\0') != npos)
        {
...
#undef zstring_view_find
  ...
  private:
    ...
    inline constexpr explicit basic_zstring_view(unsafe,
