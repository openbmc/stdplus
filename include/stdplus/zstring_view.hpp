...
inline constexpr auto
    unsafe_zstring_view(std::basic_string_view<CharT, Traits> sv)
{
    return basic_zstring_view<CharT, Traits>(sv);
}
} // namespace detail
...
#undef zstring_view_find
    ...
    inline constexpr basic_zstring_view suffix(size_type pos = 0) const
    {
        if (pos > sv.size())
        {
            throw std::out_of_range("stdplus::zstring_view");
        }
        return basic_zstring_view(sv.substr(pos));
    }

  private:
    string_view_base sv;

    inline constexpr explicit basic_zstring_view(string_view_base sv) noexcept :
        sv(sv)
    {
    }
                          ...
                          basic_zstring_view<CharT, Traits> rhs) noexcept
{
    return lhs.size() == rhs.size() &&
           lhs.compare(
               static_cast<std::basic_string_view<CharT, Traits>>(rhs)) == 0;
}
template <class CharT, class Traits>
...
inline constexpr auto
    operator<=>(basic_zstring_view<CharT, Traits> lhs,
                basic_zstring_view<CharT, Traits> rhs) noexcept
{
    return lhs.compare(static_cast<std::basic_string_view<CharT, Traits>>(rhs));
}
template <class CharT, class Traits>
...
#define zstring_all(char_t, pfx)                                               \
    ...
    {                                                                          \
    inline constexpr auto                                                      \
        operator"" _zsv(const char_t* str,                                     \
                        basic_zstring_view<char_t>::size_type size) noexcept   \
    {                                                                          \
        return detail::unsafe_zstring_view(std::basic_string_view(str, size)); \
    }                                                                          \
    }                                                                          \
    }                                                                          \
    namespace std                                                              \
